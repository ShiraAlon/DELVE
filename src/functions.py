...
def diffusion_map(X, adaptive=5500, coifman_lafon = False, alpha = 1):
    if adaptive is not None:
        K = Kernel_matrix(X, adaptive, coifman_lafon = False, alpha = 1)
    else:
        # Pairwise Euclidean distances
        pairwise_dists = squareform(pdist(X, metric='euclidean'))
        # Kernel scale ε = median of distances (common practice)
...
def calc_differential_vec_cutoff(L_A, v_B,c, k, Q=None):

    d, v = np.linalg.eig(L_A)
    L_A_c = d* v[:,:c]@v[:,:c].T
    U1 = v_B[:,:k]
    Q1 = U1 @ U1.T
    Q1 = np.eye(Q1.shape[0]) - Q1
